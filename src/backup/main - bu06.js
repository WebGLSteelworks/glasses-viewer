	...
	materials.forEach((m) => {
	  ...
	  const isAnimated = m.name?.toLowerCase().includes("anim");
		...
		if (isGlass) {
		  ...
		  if (isWebGPU) {
			// Leer opacity directamente del GLB (controla desde Blender con Alpha)
			// Si el GLB tiene opacity:1 y transmission:0, guardamos un fallback de 0.7
			const glbOpacity = (m.opacity < 1.0) ? m.opacity : 0.7;
			m.transmission = 0;
